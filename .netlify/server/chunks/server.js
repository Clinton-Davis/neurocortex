	...
	*/
	...
	#process() {
		...
		*/
		...
		for (const root of roots) this.#traverse(root, effects, render_effects);
		current_batch = null;
		if (updates.length > 0) {
			...
			for (const batch of batches) {
				...
				if (others.length > 0) {
					...
					if (batch.#roots.length > 0) {
						batch.apply();
						for (const root of batch.#roots) batch.#traverse(root, [], []);
					}
					batch.deactivate();
				}
			}
...
//#endregion
export { attr as C, experimental_async_required as D, async_mode_flag as E, noop as O, asClassComponent as S, escape_html as T, hydratable_serialization_failed as _, render as a, readable as b, get_user_code_location as c, getAllContexts as d, getContext as f, hydratable_clobbering as g, ssr_context as h, head as i, run as k, get_render_context as l, setContext as m, derived as n, slot as o, hasContext as p, ensure_array_like as r, stringify as s, attr_class as t, createContext as u, lifecycle_function_unavailable as v, clsx$1 as w, writable as x, getAbortSignal as y };
